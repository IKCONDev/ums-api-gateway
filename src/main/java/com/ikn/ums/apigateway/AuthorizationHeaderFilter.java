	...
	public GatewayFilter apply(Config config) {
		...
		return (exchange, chain) -> {
			...
			if (!isJwtValid(jwtToken)) {
				log.info(
						"AuthorizationHeaderFilter.apply() :  Authorization header retrieved from request object is not a valid one - UNAUTHORIZED");
...
//	   }
	 ...
	 */
	/**
	 * private boolean isJwtExpired(String jwtToken) { Date expiresAt = null;
	 * if(jwtToken != null && jwtToken != "") { expiresAt =
	 * Jwts.parser().setSigningKey(env.getProperty("token.secret"))
	 * .parseClaimsJwt(jwtToken) .getBody() .getExpiration(); }
	 * System.out.println(expiresAt); return expiresAt.before(new Date()); }
	 */
	...
	private boolean isJwtValid(String jwtToken) {
		...
		try {
			...
			userId = Jwts.parser().setSigningKey(env.getProperty("token.secret")).parseClaimsJws(jwtToken).getBody()
					.getSubject();
			...
			returnValue = false;
			log.error("AuthorizationHeaderFilter.isJwtValid() : Exception occured while validating jwt token "
					+ e.getMessage(), e);
			return returnValue;
